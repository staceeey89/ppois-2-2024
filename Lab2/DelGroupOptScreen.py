...
class DelGroupOptScreen(QWidget):
    def __init__(self, data_base, controller, xml):
        ...
        Design.design_input_field(self.input_group_field, 160, 173, 160, 30)
        ...
        Design.design_text(self.group_text, 160, 153, "Введите номер группы:",
                           15, 170, 17)
    ...
    def print_del(self, data_base, controller, xml):
        try:
            if not controller.check_bd_or_xml_file():
                del_num = data_base.delete_group_opt(self.input_group_field.text(),
                                    int(self.input_low_opt_field.text()), int(self.input_high_opt_field.text()))
            else:
                del_num = xml.delete_users_group_opt(self.input_group_field.text(),
                                           int(self.input_low_opt_field.text()), int(self.input_high_opt_field.text()))
            if del_num == 0:
