...
class SearchDialog:
    def __init__(self, controller):
        ...
        self.root = Tk()
        self.root.title("Поиск студентов")
        ...
        self.table = ShowTable(self.table_frame, controller,None)
    ...
    def __close(self):
        self.controller.close_page("SearchDialog")
        self.root.destroy()
    ...
    def toggle_search_widget(self, value):
        ...
        if value == 1:
            self.search_widget = WidgetSearchByGroup(self.controller,mode='search',tablewidget=self.table,master=self.root)
            self.search_widget.pack()
        if value == 2:
            self.search_widget = WidgetSubgectGrade(self.controller,mode='search',tablewidget=self.table,master=self.root)
            self.search_widget.pack()
        elif value == 3:
            self.search_widget = WidgetGPASubgect(self.controller,mode='search',tablewidget=self.table,master=self.root)
            self.search_widget.pack()
