...
class WidgetSubgectGrade(tk.Frame):
    def __init__(self,controller,mode='search',tablewidget = None, master=None, **kwargs):
        ...
        self.mode = mode
        self.min_label = tk.Label(self, text="Минимальный балл по предмету:")
        self.min_entry = tk.Entry(self)
        self.min_label.grid(row=0, column=0, sticky="w")
        self.min_entry.grid(row=0, column=1, sticky="w")

        self.max_label = tk.Label(self, text="Максимальный балл по предмету:")
    ...
    def find(self)->List[int]:
        try:
            ...
            if selected_item:
                ...
                list_id= self.controller.database.get_students_by_score_range(min_grade, max_grade, selected_item)
                if len(list_id) == 0:
